...
algorithm, sample, feature and daily/hourly/streaming experiments are enumerated
in the [Launch Ladder](docs/plans/digital-twin-v4-launch-ladder.md).
P1, the Feed scope of P2, and retrieval through P3-06 pass. The v4 request
samples feed persistent active/candidate lanes and the content-bound registry
enforces checkpoint, index and fallback compatibility. The feature manifest is
accepted, but full ranking-input tensor coverage and propensity support reopen
as P3-02a/03a/05a. P3-06 compares lifecycle, Graph,
Two-Tower and Multi-interest retrieval on the same persisted requests, corpus,
Top-K and latency budget. Its 100K/1M review retains lifecycle control and
rejects every challenger because old-policy exposures collapse target diversity;
P3-09a and the reopened contracts now precede coarse/fine model ladders.

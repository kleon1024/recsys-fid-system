...
training feature. Each Feed exposure predicts whether the viewer enters
posting, creates or publishes within the task window; multiple exposures may
share one later outcome because this is a conditional response label, not a
claim of unique causation. Incremental value is established by viewer-UID randomized Feed
...
| P3-05 | `platform/features`: one feature/FID manifest for train and serve | Done for request trace and replay: full recall-universe tensors are retained | P3-03 | learned model ladders must consume the accepted bytes without a parallel feature path |
...
| P3-10 | `samples/publish_queue` + independent Feed Publish Queue | Cross-request PIT authority implemented for posting-entry/create/publish exposure-window labels with maturity masks and propensity; dense/sparse LR and mixer runner implemented, launch pending | P1-02, P2 creator response, P3-03/04/05 | add qualified-post head; train and shadow a separate content scorer; launch viewer-UID A/B through the common mixer without reading hidden inspiration or relabeling posting-page candidates |
...
Background arrivals cannot claim product lift. The Feed Publish team owns an
independent content queue over the same Feed candidates and predicts whether a
viewer exposure causes later `Posting entry -> Create -> Publish -> qualified
post`; it may share embeddings with consumption ranking, but owns different
samples, maturity windows and scores before the common mixer. This exposure
policy is viewer-UID assigned. Creator-assigned experiments are separate and
measure incentives or distribution feedback changing creator motivation,
creation cost, retention and subsequent supply.
...
Status: Commerce workflow implementation is in validation; other business
workflows remain skeletons. C-LR-001 ended without support and C-LR-002 exposed
cross-route eligibility leakage. C-LR-003 is the first corrected global
inventory experiment and remains unclaimed until the RTX run completes.

...
| Feed retrieval mechanics | Random and Popular are factual baselines; lifecycle/Graph plus registry-backed Two-Tower/Multi-interest serving paths exist | Popular promoted; cold-start stopped inconclusive; recent ANN pending | Continue sequential fixed-ranker LRs before learned retrieval |
| Durable evolving world | Content-addressed hidden world, supply, delayed queue, platform, index, event, experiment and learning cursor checkpoint | Implemented and RTX-verified | Incremental/lazy event-log compaction remains a scale optimization |
...
| Model ladder | Sequential factual retrieval ladder resumes from world checkpoints under one budget | Popular promoted; cold-start stopped inconclusive; ANN pending | Coarse/fine and learned retrieval remain P3-07..09 |
...
| P3-06 | `platform/retrieval` + `learning/retrieval`: migrate retrieval ladder | Sequential factual mainline active: Popular promoted, cold-start stopped inconclusive, ANN pending | P3-01/04/05 | continue fixed-corpus/Top-K windows from durable checkpoints; learned retrieval follows accepted direct fine baseline |
